...
export default function RadialDendrogram({ data, onNodeClick, width = 900, height = 900 }) {
    ...
    nodeGroup.filter(d => d.depth > 0)
      ...
      .on("mouseover", function(event, d) {
        ...
        tooltip
          ...
          .html(`
            <span style="font-weight:600;color:${color}">${d.data.name.replace(/\n/g, " ")}</span>
            ${d.children ? `<br/><span style="color:rgba(255,255,255,0.4);font-size:10px">${d.children.length} children</span>` : ''}
            ${d.data.year ? `<br/><span style="color:rgba(255,255,255,0.3);font-size:10px;font-family:'JetBrains Mono',monospace">${d.data.year}</span>` : ''}
            <br/><span style="color:rgba(100,140,255,0.6);font-size:9px">Click to explore</span>
          `);
      })
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', maxHeight: 'calc(100vh - 160px)' }}>
      <svg ref={svgRef} width="100%" height="100%" style={{ maxWidth: '100%', minHeight: '500px' }} />
